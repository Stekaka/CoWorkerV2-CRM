...
import { Search, Plus, Edit, Trash2, DollarSign, Package, CheckCircle, ArrowLeft } from 'lucide-react'
import Link from 'next/link'
import StandaloneBottomNav from '../components/StandaloneBottomNav'
import { getOrderStats, Order } from '../../../lib/shared-data'
...
export default function OrdersPage() {
  ...
  // Use shared stats function for consistency
  const stats = getOrderStats(orders)

  const handleSaveOrder = () => {
    if (!formData.customer_name || !formData.order_number || !formData.amount) {
          ...
          <div className="overflow-x-auto">
                ...
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-800/50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {order.description && (
                        <div className="text-sm text-slate-400 truncate max-w-xs">{order.description}</div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-slate-300">{order.customer_name}</td>
            ...
            {filteredOrders.length === 0 && (
              <div className="text-center py-8">
                <p className="text-slate-400">Inga orders hittades</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Kundnamn *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Ordernummer *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Belopp *</label>
                <input
                  type="number"
